    ...
    return app.run(ac, av, [&] {
        return seastar::async([&] {
            ...
            for (auto eval: mountpoint_map) {
                ...
                desc.num_io_queues = std::min(smp::count, unsigned((task_quotas_in_default_latency_goal * desc.write_iops * latency_goal) / 4));
                desc.num_io_queues = std::min(desc.num_io_queues, unsigned((task_quotas_in_default_latency_goal * desc.write_bw * latency_goal) / (4 * 4096)));
                desc.num_io_queues = std::max(desc.num_io_queues, 1u);
                fmt::print("Recommended num_io_queues: {}\n", desc.num_io_queues);
